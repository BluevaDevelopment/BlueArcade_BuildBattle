...
public class OptionsMenuRepository {
    ...
    public static final String MENU_MAIN = "options_main";
    public static final String MENU_TIME = "options_time";
    public static final String MENU_WEATHER = "options_weather";

    private final ModuleConfigAPI moduleConfig;
    private final Map<String, MenuDefinition<Material>> menus = new HashMap<>();

    public OptionsMenuRepository(ModuleConfigAPI moduleConfig) {
        this.moduleConfig = moduleConfig;
    }

    public void loadMenus() {
        menus.clear();
        loadMenu(MENU_MAIN, "menus/java/build_battle_options_main.yml",
                "menus/bedrock/build_battle_options_main.yml");
        loadMenu(MENU_TIME, "menus/java/build_battle_options_time.yml",
                "menus/bedrock/build_battle_options_time.yml");
        loadMenu(MENU_WEATHER, "menus/java/build_battle_options_weather.yml",
                "menus/bedrock/build_battle_options_weather.yml");
    }
    ...
    private void loadMenu(String id, String javaFile, String bedrockFile) {
        ...
        String title = stringOrEmpty(javaConfig.getString("menuName"));
        int size = javaConfig.contains("menuSize") ? javaConfig.getInt("menuSize") : 9;
    ...
    private List<JavaMenuItem<Material>> loadJavaItems(ConfigurationSection section) {
        ...
        for (String key : section.getKeys(false)) {
            ...
            int slot = itemSection.contains("slot") ? itemSection.getInt("slot") : -1;
            String name = stringOrEmpty(itemSection.getString("name"));
            ...
            int amount = Math.max(1, stackSection.contains("amount") ? stackSection.getInt("amount") : 1);
    ...
    private BedrockMenuDefinition loadBedrockMenu(String bedrockFile) {
        ...
        String title = stringOrEmpty(bedrockConfig.getString("menuName"));
        ...
        if (buttonsSection != null) {
            for (String key : buttonsSection.getKeys(false)) {
                ...
                String text = stringOrEmpty(button.getString("text"));
